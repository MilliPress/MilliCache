<?php
/**
 * Logger class for MilliCache\Deps\MilliRules
 *
...
class Logger {
	...
	private function log_summary( array $cache_entry ): void {
		...
		if ( $count > 1 ) {
			$summary = sprintf(
				'MilliCache\Deps\MilliRules [INFO]: Previous message repeated %d times over %d seconds',
				$count - 1,
				$time_elapsed
			);
			error_log( $summary );
	...
	private function log( int $level, string $message, array $context = array() ): void {
		...
		$formatted_msg  = "MilliCache\Deps\MilliRules [{$level_name}]: {$message}";
