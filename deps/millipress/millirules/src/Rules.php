...
use MilliCache\Deps\MilliRules\Logger;
use MilliCache\Deps\MilliRules\Builders\ConditionBuilder;
use MilliCache\Deps\MilliRules\Builders\ActionBuilder;
use MilliCache\Deps\MilliRules\Packages\PackageManager;
/**
 * Class Rules
 *
...
class Rules
{
     ...
     */
    private static array $namespace_package_map = array('MilliCache\Deps\MilliRules\Packages\PHP\\' => 'PHP', 'MilliCache\Deps\MilliRules\Packages\WordPress\\' => 'WP', 'MilliCache\Deps\MilliRules\Conditions\\' => 'Core', 'MilliCache\Deps\MilliRules\Actions\\' => 'Core');
    /**
    ...
    {
        $instance = new self();
        $instance->rule = array('id' => $id, 'title' => '', 'order' => 10, 'enabled' => true, 'match_type' => 'all', 'conditions' => array(), 'actions' => array(), '_metadata' => array());
    ...
    public static function register_condition(string $type, callable $callback): void
    {
        if (!is_callable($callback)) {
            throw new \InvalidArgumentException("Callback for condition type '{$type}' is not callable");
    ...
    {
        if (!is_callable($callback)) {
            throw new \InvalidArgumentException("Callback for action type '{$type}' is not callable");
            // phpcs:ignore WordPress.Security.EscapeOutput
        }
    ...
    {
        return \MilliCache\Deps\MilliRules\Conditions\BaseCondition::compare_values($actual, $expected, $operator);
    }
    // ===========================
    // Rule Builder API
    // ===========================
    ...
    public function register(): bool
    {
        ...
        // Build metadata.
        $metadata = array('required_packages' => $required_packages, 'type' => $type, 'order' => $this->rule['order'], 'enabled' => $this->rule['enabled']);
    ...
    private function validate_type_conflicts(string $type, array $required_packages): string
    {
        ...
        if ($this->explicit_type === 'php' && in_array('WordPress', $required_packages, true)) {
            Logger::warning("Rule '{$rule_id}' has explicit type='php' but requires WordPress package - " . 'this may not work as expected. Consider using type=\'wp\' or omitting type parameter for auto-detection.');
        ...
        if ($this->explicit_type === 'php' && ($this->hook !== 'wp' || $this->hook_priority !== 10)) {
            Logger::warning("Rule '{$rule_id}' has explicit type='php' but uses WordPress hook '{$this->hook}' - " . 'auto-changing type to \'wp\' as hooks require WordPress context.');
